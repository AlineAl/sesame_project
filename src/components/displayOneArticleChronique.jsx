...
function DisplayOneArticleChronique() {
    ...
    const GetOneArticleChronique = async () => {
        try {
            axios.get(`https://sesameoeuvretoiadmin.herokuapp.com/api/articles-chroniques/${id}?populate=*`)
            .then((response) => {
           ...
           <nav>
            ...
            <section className="container-find-one">
                ...
                <div className="text-find-one">
                    <p className="date-academie">{article.attributes.dateChronique}</p>
                    {(article.attributes.imageChronique.data) === null ? '' : <img id="img-academie-article" src={article.attributes.imageChronique.data[0].attributes.url} alt="image qui illustre le détail de l'article" />}
